...
#         'input_chunk_length': int(random.uniform(40,150)),
#     }
...
#     model_random = NBEATSModel(**hyperparameters, random_state=0,output_chunk_length=7)
...
NBEATS_example = NBEATSModel(
    input_chunk_length=49, output_chunk_length=7, n_epochs=30, random_state=0, 
)
NBEATS_example.fit([Target_series, 
                    Co_variate_EU1_recycling,Co_variate_EU2_recycling], verbose=True)

#%% prejection beyond dataset

pred_NBEATS = NBEATS_example.predict(36,  series=Target_series)

#%% NBEATS test period
NBEATS_example_train = NBEATSModel(
    input_chunk_length=49, output_chunk_length=7, n_epochs=30, random_state=0,
)


NBEATS_example_train.fit([Target_series_train,
                          Co_variate_EU1_recycling_train,Co_variate_EU2_recycling_train], verbose=True)

#%% prejection test period

pred_NBEATS_train = NBEATS_example_train.predict(36, series=Target_series_train)
...
pred_NBEATS_train = pred_NBEATS_train*100
pred_NBEATS = pred_NBEATS*100
scientific_colors = {
...
ax.legend(loc='lower center', fontsize=14,frameon=True, framealpha=0.7)
