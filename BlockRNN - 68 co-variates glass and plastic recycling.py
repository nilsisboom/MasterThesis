# -*- coding: utf-8 -*-
"""
Created on Wed Nov 15 21:19:56 2023
...
def random_search():
    hyperparameters = {
        ...
        'n_epochs':int(random.uniform(80,140)),
        'input_chunk_length': int(random.uniform(50,70)),
        'output_chunk_length': int(random.uniform(25,40))
    }
...
RNN_full = BlockRNNModel(
   model='LSTM', input_chunk_length=58, output_chunk_length=38, n_epochs=116, random_state=0
)

RNN_full.fit([Target_series, Co_variate_Glas_recycling, Co_variate_Plastic_recycling, 
...
pred_RNN = RNN_full.predict(40, series=Target_series)

#%% Train model with test dataset (for test period forecast)

RNN_train = BlockRNNModel(
    input_chunk_length=58, output_chunk_length=38, n_epochs=116, 
    random_state=0
)

RNN_train.fit([Target_series_train , Co_variate_EU1_recycling_train, 
                 Co_variate_EU2_recycling_train, Co_variate_Glas_recycling_train, 
                 Co_variate_Plastic_recycling_train,], verbose=True)

#%% NBEATS test period prediction

pred_RNN_train = RNN_train.predict(40,  series=Target_series_train)
...
print('mae',mean_absolute_error(Target_series_val*100, test_forecast*100))
print('MAPE',mean_absolute_percentage_error(Target_series_val*100, test_forecast*100))
print('R2',r2_score(Target_series_val*100, test_forecast*100) )
print('mse',mean_squared_error(Target_series_val*100, test_forecast*100))
print('rsme',sqrt(mean_squared_error(Target_series_val*100, test_forecast*100)))
...
print('projected recycling rate', predicted_recycling_rate*100)
