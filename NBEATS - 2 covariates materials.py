...
#     r2 = r2_score(Target_series_val['Recycling quote'], pred)      
...
NBEATS_example = NBEATSModel(
    input_chunk_length=27, output_chunk_length=10, n_epochs=144, random_state=0,
)
...
decomposition = seasonal_decompose(pred_df_mean)
...
rmse = np.sqrt(np.mean((pred_df_mean - Target_series_val['Recycling quote'].values) ** 2))

# Calculate MAPE
from sklearn.metrics import mean_absolute_error
print(mean_absolute_error(Target_series_val['Recycling quote'].values, pred_df_mean))


# Calculate MAPE
ape = np.abs((pred_df_mean - Target_series_val['Recycling quote'].values) / Target_series_val['Recycling quote'].values)
mape = np.mean(ape) * 100

from sklearn.metrics import r2_score
print(r2_score(Target_series_val['Recycling quote'].values, pred_df_mean))
...
sse = np.sum((Target_series_val['Recycling quote'].values - pred_df_mean) ** 2)
r2 = 1 - (sse / tss)
