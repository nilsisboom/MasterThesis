# -*- coding: utf-8 -*-
"""
Created on Wed Nov 15 11:24:45 2023
...
#%% read in EWC and WEEE Eurostat data
...
#%% filter per wastestream and year
...
#%% add all metal frames together
# as a general metal waste treatment rate is of interesst
...
df.at[df.index[1], highest_index_label] = df_average
...
interpolated_co_variate_1[interpolated_co_variate_1 > 1] = 1
interpolated_co_variate_1[interpolated_co_variate_1 < 0] = 0
...
df.at[df.index[1], second_highest_index_label] = df_average
...
interpolated_co_variate_2[interpolated_co_variate_2 > 1] = 1
interpolated_co_variate_2[interpolated_co_variate_2 < 0] = 0
...
two_recycling_past_co_variates_series_train, two_recycling_past_co_variates_series_val = two_recycling_past_co_variates_series[:-40], two_recycling_past_co_variates_series[-36:] 
Co_variate_recycling_train, Co_variate_recycling_test = two_recycling_past_co_variates_series[:-40], two_recycling_past_co_variates_series[-36:]
...
def random_search():
    hyperparameters = {
        ...
        'n_epochs':int(random.uniform(70,140)),
        'input_chunk_length': int(random.uniform(50,60)),
        'output_chunk_length': int(random.uniform(40,50))
    }
...
#                     Co_variate_EU1_recycling_train,Co_variate_EU2_recycling_train], verbose=True)
...
#     Mape = mean_absolute_percentage_error(Target_series_val['Recycling quote']*100, pred.values*100)
...
RNN_example = BlockRNNModel(
    model="LSTM",input_chunk_length=59, output_chunk_length=46, n_epochs=129, random_state=0, 
)
...
#adopt out of sample forcast for plot
#%%
pred_out_of_sample = pred_RNN.pd_dataframe()
target_series_df = Target_series.pd_dataframe()
last_value_=pd.DataFrame(target_series_df.iloc[-1,:]).transpose()
...
#%% Plot the forecast in test period and in out-of-sample period
Target_series = Target_series*100
pred_RNN_train = pred_RNN_train*100
pred_RNN=pred_RNN*100
Co_variate_EU1_recycling_train=Co_variate_EU1_recycling_train*100
Co_variate_EU2_recycling_train = Co_variate_EU2_recycling_train*100
...
Co_variate_EU1_recycling_train.plot(
    label='Co-variate fluorescent lamps Luxembourg',
    ax=ax,
    color="mediumpurple",
    linestyle=':',
    linewidth=2
...
Co_variate_EU2_recycling_train.plot(
    label='Co-variate fluorescent lamps Germany',
    ax=ax,
    color="darkkhaki",
    linestyle=':',
    linewidth=2
...
#%% Plot percentual difference of forecast and test data
import matplotlib.patches as patches

performance_values_recycling = ((pred_df.values - Target_series_val['Recycling quote'].values) /  Target_series_val['Recycling quote'].values) * 100
...
plt.xlabel('Percentual difference from test data [%]',fontsize=16)
plt.ylabel('Steps into the test set',fontsize=16)
...
ax.tick_params(axis='both', labelsize=14)  # Adjust the fontsize as needed

# Display the plot
plt.show()


#%% Moel quality metrics
from sklearn.metrics import mean_absolute_error, mean_absolute_percentage_error
from sklearn.metrics import r2_score
from sklearn.metrics import mean_squared_error
...
print('mae',mean_absolute_error(Target_series_val['Recycling quote']*100, test_forecast))
print('MAPE',mean_absolute_percentage_error(Target_series_val['Recycling quote']*100, test_forecast))
print('R2',r2_score(Target_series_val['Recycling quote']*100, test_forecast) )
print('mse',mean_squared_error(Target_series_val['Recycling quote']*100, test_forecast))
print('rsme',sqrt(mean_squared_error(Target_series_val['Recycling quote']*100, test_forecast)))
...
print('historic recycling rate',historic_recycling_rate)
